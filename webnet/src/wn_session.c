 ...
 */
int webnet_session_write(struct webnet_session *session, const uint8_t* data, wm_size_t size)
{
    /* send data directly */
...
#ifdef CONFIG_COMPONENT_CLI_ENABLED
#include "wm_cli.h"

static void list_webnet(int argc, char *argv[])
{
    struct webnet_session *session;
...
WM_CLI_CMD_DEFINE(list_webnet, list_webnet, list webnet, list_webnet);
