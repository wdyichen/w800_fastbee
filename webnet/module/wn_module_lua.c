...
#if defined(CONFIG_WEBNET_USING_LUA) && defined(WM_USING_LUA)
...
int webnet_module_lua(struct webnet_session* session, int event)
{
    ...
    if (event == WEBNET_EVENT_URI_POST)
    {
        ...
        if ((strstr(request->path, ".lua") != NULL) ||
                (strstr(request->path, ".LUA") != NULL))
        {
        ...
        else
        {
            ...
            if (lua_filename != NULL)
            {
                snprintf(lua_filename, WEBNET_PATH_MAX, "%s/index.lua", request->path);
