...
struct webnet_module_put_entry
{
    ...
    int (*put_write)(struct webnet_session* session, const void* data, wm_size_t length);
...
int webnet_module_dav(struct webnet_session* session, int event)
{
    ...
    if(session->request->method == WEBNET_OPTIONS)
    {
        ...
        wm_log_debug("OPTIONS %s", session->request->path);
    ...
    else if(session->request->method == WEBNET_PROPFIND)
    {
        ...
        wm_log_debug("PROPFIND %s depth: %s", session->request->path,
        ...
        if(!exit_file)
        {
            wm_log_error("Open file(%s) is not exist.", session->request->path);
    ...
    {
        wm_log_debug("PUT %s", session->request->path);
    ...
    else if(session->request->method == WEBNET_PROPPATCH)
    {
        ...
        wm_log_debug("PROPPATCH %s", session->request->path);
    ...
    {
        wm_log_debug("DELETE %s", session->request->path);
    ...
    else if (session->request->method == WEBNET_MKCOL)
    {
        ...
        wm_log_debug("MKCOL %s", session->request->path);

        ret = mkdir(session->request->path, 0x777);
        if (ret < 0)
        {
            session->request->result_code = 404;
            wm_log_error("MKCOL mkdir error, path %s.", session->request->path);
        }
        else
        {
            wm_log_debug("MKCOL mkdir ok !");
        }
    ...
    else if (session->request->method == WEBNET_MOVE)
    {
        ...
        if (path)
        {
            ...
            if (full_path)
            {
                ...
                wm_log_debug("Get full path, %s => %s\n", session->request->path, full_path);
...
_exit:
    wm_log_error(" %s failed  ,file:%s  ,line:%d",__FUNCTION__,__FILE__,__LINE__);
    return (int)fd;
}
...
static int put_write(struct webnet_session* session, const void* data, wm_size_t length)
{
    int fd;
...
static void _webnet_module_put_handle(struct webnet_session* session, int event)
{
    if(session->request->method == WEBNET_PUT)
    {
        ...
        if (length <= 0)
        {
            wm_log_error(" %s failed  ,file:%s  ,line:%d\n",__FUNCTION__,__FILE__,__LINE__);
...
int webnet_module_put_method(struct webnet_session* session)
{
    ...
    {
        wm_log_error("No memory for module put session.");
        return WEBNET_MODULE_CONTINUE;
    }
