...
#if (!CONFIG_WEBNET_FS)
struct webnet_file_items asp_file__item;
#endif
...
void webnet_asp_add_var(const char* name, void (*handler)(struct webnet_session* session, void* handler_priv),
                        void* priv)
{
    ...
    _webnet_asp_vars[_webnet_asp_vars_count - 1].handler_priv = priv;
}
...
static void _default_asp_handler(struct webnet_session* session, const char* name)
{
    ...
    {
        webnet_session_printf(session, "%d", CONFIG_WEBNET_PORT);
    }
...
#else
    length = asp_file__item.file_size;
#endif
...
#else
    offset = (char *)asp_file__item.file_content;
    end = (char *)asp_file__item.file_content + length;
#endif
    while (offset < end)
    {
        ...
        else
        {
            ...
            for (index = 0; index < _webnet_asp_vars_count; index ++)
            {
                ...
                {
                    /* found asp variable */
                    _webnet_asp_vars[index].handler(session, _webnet_asp_vars[index].handler_priv);
                    break;
                }
            }
...
int webnet_module_asp(struct webnet_session* session, int event)
{
    ...
    char *str = NULL;
                ...
                (strstr(request->path, ".ASP") != NULL))
        {
            ...
            if(NULL != str)
            {
                wn_get_file_information(str, &asp_file__item);
                _webnet_asp_dofile(session, 1);
                return WEBNET_MODULE_FINISHED;
            }
            else
            {
